...
void RiverMetaTranslator::MakeAddNoFlagsRegImm8(RiverInstruction *rOut, const RiverRegister &reg, unsigned char offset, BYTE family, DWORD addr, rev::DWORD index) {
...
void RiverMetaTranslator::MakeSubNoFlagsRegImm8(RiverInstruction *rOut, const RiverRegister &reg, unsigned char offset, BYTE family, DWORD addr, rev::DWORD index) {
	unsigned char rg = (reg.name & 0x07) | RIVER_OPSIZE_32;
...
void RiverMetaTranslator::MakeMovRegMem32(RiverInstruction *rOut, const RiverRegister &reg, const RiverAddress &mem, BYTE family, DWORD addr, rev::DWORD index) {
	rOut->opCode = 0x8B;
	rOut->subOpCode = 0;
	rOut->modifiers = 0;
	rOut->family = family;
...
void RiverMetaTranslator::MakeMovMemReg32(RiverInstruction *rOut, const RiverAddress &mem, const RiverRegister &reg, BYTE family, DWORD addr, rev::DWORD index) {
	rOut->opCode = 0x89;
	rOut->subOpCode = 0;
	rOut->modifiers = 0;
	rOut->family = family;
...
void RiverMetaTranslator::MakeMovMemImm32(RiverInstruction *rOut, const RiverAddress &mem, DWORD imm, BYTE family, DWORD addr, rev::DWORD index) {
	rOut->opCode = 0xC7;
	rOut->subOpCode = 0;
	rOut->modifiers = 0;
	rOut->family = family;
...
void RiverMetaTranslator::MakeMovMemMem32(RiverInstruction *rOut, const RiverAddress &memd, const RiverAddress &mems, BYTE family, DWORD addr, rev::DWORD index) {
	rOut->opCode = 0xA5;
	rOut->subOpCode = 0;
	rOut->modifiers = 0;
	rOut->family = family;
...
void RiverMetaTranslator::TranslatePushReg(RiverInstruction *rOut, const RiverInstruction &rIn, DWORD &instrCount) {
	MakeMovMemReg32(&rOut[0], *(rIn.operands[2].asAddress), rIn.operands[0].asRegister, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
	MakeSubNoFlagsRegImm8(&rOut[1], rIn.operands[1].asRegister, 0x04, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
...
void RiverMetaTranslator::TranslatePushMem(RiverInstruction *rOut, const RiverInstruction &rIn, DWORD &instrCount) {
	MakeMovMemMem32(&rOut[0], *(rIn.operands[2].asAddress), *(rIn.operands[0].asAddress), RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
	MakeSubNoFlagsRegImm8(&rOut[1], rIn.operands[1].asRegister, 0x04, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
	CopyInstruction(rOut[2], rIn);
	rOut[2].family |= RIVER_FAMILY_FLAG_METAPROCESSED;
	instrCount += 3;
...
void RiverMetaTranslator::TranslatePusha(RiverInstruction *rOut, const RiverInstruction &rIn, DWORD &instrCount) {
	...
	MakeMovMemReg32(&rOut[0], rStack, rReg, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);

	rStack.disp.d8 = 0xF8;
	rReg.versioned = codegen->GetCurrentReg(RIVER_REG_xCX);
	MakeMovMemReg32(&rOut[1], rStack, rReg, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);

	rStack.disp.d8 = 0xF4;
	rReg.versioned = codegen->GetCurrentReg(RIVER_REG_xDX);
	MakeMovMemReg32(&rOut[2], rStack, rReg, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);

	rStack.disp.d8 = 0xF0;
	rReg.versioned = codegen->GetCurrentReg(RIVER_REG_xBX);
	MakeMovMemReg32(&rOut[3], rStack, rReg, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);

	rStack.disp.d8 = 0xEC;
	rReg.versioned = codegen->GetPrevReg(RIVER_REG_xSP);
	MakeMovMemReg32(&rOut[4], rStack, rReg, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);

	rStack.disp.d8 = 0xE8;
	rReg.versioned = codegen->GetCurrentReg(RIVER_REG_xBP);
	MakeMovMemReg32(&rOut[5], rStack, rReg, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);

	rStack.disp.d8 = 0xE4;
	rReg.versioned = codegen->GetCurrentReg(RIVER_REG_xSI);
	MakeMovMemReg32(&rOut[6], rStack, rReg, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);

	rStack.disp.d8 = 0xE0;
	rReg.versioned = codegen->GetCurrentReg(RIVER_REG_xDI);
	MakeMovMemReg32(&rOut[7], rStack, rReg, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);

	rReg.versioned = codegen->GetCurrentReg(RIVER_REG_xSP);
	MakeSubNoFlagsRegImm8(&rOut[8], rReg, 0x20, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
...
void RiverMetaTranslator::TranslatePopReg(RiverInstruction *rOut, const RiverInstruction &rIn, DWORD &instrCount) {
	MakeMovRegMem32(&rOut[0], rIn.operands[0].asRegister, *rIn.operands[2].asAddress, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
	MakeAddNoFlagsRegImm8(&rOut[1], rIn.operands[1].asRegister, 0x04, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
...
void RiverMetaTranslator::TranslatePopMem(RiverInstruction *rOut, const RiverInstruction &rIn, DWORD &instrCount) {
	MakeMovMemMem32(&rOut[0], *rIn.operands[0].asAddress, *rIn.operands[2].asAddress, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
	MakeAddNoFlagsRegImm8(&rOut[1], rIn.operands[1].asRegister, 0x04, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
...
void RiverMetaTranslator::TranslateCall(RiverInstruction *rOut, const RiverInstruction &rIn, DWORD &instrCount) {
	MakeMovMemImm32(&rOut[0], *(rIn.operands[3].asAddress), rIn.operands[1].asImm32, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
	MakeSubNoFlagsRegImm8(&rOut[1], rIn.operands[2].asRegister, 0x04, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
	CopyInstruction(rOut[2], rIn);
	rOut[2].family |= RIVER_FAMILY_FLAG_METAPROCESSED;
	instrCount += 3;
}
...
void RiverMetaTranslator::TranslateRet(RiverInstruction *rOut, const RiverInstruction &rIn, DWORD &instrCount) {
	MakeAddNoFlagsRegImm8(&rOut[0], rIn.operands[0].asRegister, 0x04, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
...
void RiverMetaTranslator::TranslateRetn(RiverInstruction *rOut, const RiverInstruction &rIn, DWORD &instrCount) {
	MakeAddNoFlagsRegImm8(&rOut[0], rIn.operands[1].asRegister, rIn.operands[0].asImm16 + 0x04, RIVER_FAMILY_PREMETA, rIn.instructionAddress, rIn.index);
	CopyInstruction(rOut[1], rIn);
	rOut[1].family |= RIVER_FAMILY_FLAG_METAPROCESSED;
	instrCount += 2;
}
