...
namespace revwrapper {
	...
	DLL_WRAPPER_PUBLIC extern void CallFreeMemoryHandler(void *address) {
		return freeVirtual(address);
	}

	DLL_WRAPPER_PUBLIC extern void *CallMapMemoryHandler(unsigned long mapHandler, unsigned long access, unsigned long offset, unsigned long size, void *address) {
	...
	DLL_WRAPPER_PUBLIC extern int CallFormattedPrintHandler(char * buffer, unsigned int sizeOfBuffer, const char * format, char *argptr) {
		return formatPrint(buffer, sizeOfBuffer, format, (char *)argptr);
	}

	DLL_WRAPPER_PUBLIC extern bool CallWriteFile(void *handle, void *buffer, unsigned int size, unsigned long *written) {
		return writeFile(handle, buffer, size, written);
	}

	DLL_WRAPPER_PUBLIC extern long CallConvertToSystemErrorHandler(long status) {
